...
async def test_slack_mentions_reactions() -> bool:
    ...
    sent = await backend.send_message(channel_id=channel.id, content=content)
...
async def test_discord_mentions_reactions() -> bool:
    ...
    if user:
        ...
        msg.text(f"@{user_name}")
    msg.text("! Testing mentions and reactions.")
...
async def test_symphony_mentions_reactions() -> bool:
    ...
    msg.text("Hello ")
    msg.mention(user)
    msg.text("! Testing Symphony mentions.")

    content = msg.render(backend.get_format())
    sent = await backend.send_message(channel_id=room.id, content=content)
